...
async function buildAxilumInternalBoostContext({ groqKey, question, recentHistory, logger } = {}) {
  ...
  for (const agent of agents) {
    ...
    const workerData = await callGroqChatCompletion(groqKey, workerMessages, { max_tokens: 900, temperature: 0.35 });
  ...
  let synthText = '';
  try {
    const synthData = await callGroqChatCompletion(groqKey, synthMessages, { max_tokens: 800, temperature: 0.25 });
